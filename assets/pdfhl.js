// ---------------------------------------------------------------------------
// pdfhl.js — minimal PDF.js viewer with anchored, taggable highlights.
// No build step; lazy-loads the vendored pdf.js. Geometry is stored as fractions
// of each page so highlights survive any zoom/scale.
//
...
//
// onSelect({page, rects, quote, anchor}) fires when the user selects text;
// onOpen(hl) fires when an existing highlight is clicked. The host persists.
// ---------------------------------------------------------------------------
(function () {
  ...
  async function open(opts) {
    const { container, url, scale = 1.35 } = opts;
    ...
    container.innerHTML = "";
    const pages = {};   // pageNum -> { div, w, h, hlLayer }
    ...
    (opts.highlights || []).forEach(renderHL);
    ...
    return {
      render: renderHL,
      remove(id) { container.querySelectorAll(`.pdfhl-hl[data-id="${id}"]`).forEach(n => n.remove()); },
